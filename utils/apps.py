...
import subprocess, os, random, time, json, re

from utils.constants import CONFIG_PATH, ICONS, DEFAULT_MUSIC_DIRS, MUSIC_EXTENSIONS, MUSIC_PLAYER_SETTING_CSS, MUSIC_PLAYER_CSS, FILE_EXPLORER_CSS, DIALER_CSS
from utils.helpers import load_config, save_config, scan_music, mp_run, mp_info, to_mmss, fmt_size, run_cmd, call_number, fetch_contacts, fetch_call_log, type_icon, clean_number, match_contacts
...
class DialerScreen(Screen):
    ...
    def _render_contacts(self, contacts):
        ...
        if not contacts:
            scroll.mount(Static("No contacts found."))
            return
